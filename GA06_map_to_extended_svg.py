...
tmp_file=r'GA06_block_map.xlsx'
sheet='Sheet'
start_cell='A1'
end_cell='AW43'
addText=False
...
save_file='./data_files/GA06_BW_runoff.png'

# This is the SVG file
svg_file=r'./data_files/GA06_runoff.svg'

# This is the CSV file
csv_file=r'./data_files/GA06_runoff.csv'
...
ws_blocks=wb_blocks['Precincts']
for row in range(2,210):
    precinct=ws_blocks.cell(row=row,column=7).value
    county=ws_blocks.cell(row=row,column=6).value
    precinct=re.sub('\s+$','',precinct)
    county=re.sub('\s+$','',county)
    precinct=re.sub('Rocky Mountain','Rocky Mount',precinct)
    precinct=re.sub('Lasssiter','Lassiter',precinct)
    precinct=re.sub('Shallowford Fall ','Shallowford Falls ',precinct)
    precinct=re.sub('Wileo','Willeo',precinct)
    precinct=re.sub('PLEASANTDALE ELEM','PLEASANTDALE ROAD',precinct)
    precinct_dict[precinct]=county
precinct_dict['CHAMBLEE 2']='DeKalb'
...
if addText:
    text_dict=dict()
    text_cnt=0
im=Image.new("RGB",(ncols*(xw+1)+1+4*offset_val,nrows*(yw+1)+1+3*offset_val),"white")
pix=im.load()
for col in range(ncols):
    for row in range(nrows):
        precinct=rng[row][col].value
        if(precinct is not None):
            precinct=re.sub('\s+$','',precinct)
            county=precinct_dict[precinct]
            if(county=='Cobb'):
                xoffset=-1*offset_val
                yoffset=-1*offset_val
            elif(county=='Fulton'):
                xoffset=1*offset_val
                yoffset=0*offset_val
            elif(county=='DeKalb'):
                xoffset=3*offset_val
                yoffset=2*offset_val
        else:
            offset=0
        ...
        if((R,G,B)!=(255,255,255)):
            for x in range(xw):
                for y in range(yw):
                    pix[x+1+(xw+1)*col+xoffset+offset_val,y+1+(yw+1)*row+yoffset+offset_val]=(R,G,B)
        if(rng[row][col].comment):
            key=rng[row][col].comment.text.split('\n')[0]
            key=re.sub(' ','_',key)
            comment_dict[key]=((xw/2+1)+(xw+1)*col+xoffset+offset_val,(yw/2+1)+(yw+1)*row+yoffset+offset_val)
        if(addText):
            if(rng[row][col].value):
                ...
                try:
                    ...
                    text_dict[text_cnt]['value']=rng[row][col].value
                text_dict[text_cnt]['position']=((xw/2+1)+(xw+1)*col+xoffset+offset_val,(yw/2+1)+(yw+1)*row+yoffset+offset_val)
                text_cnt=text_cnt+1
im.show()
mode=im.mode
size=im.size
